...
function RegisterPage() {
  ...
  return <div><section className="hero"><h1>성인권 교육 강사 등록</h1><p>강사가 직접 정보를 입력하는 페이지입니다. 입력 정보는 관리자 검토 후 공개됩니다.</p></section>{message ? <div className="notice">{message}</div> : null}{error ? <div className="error">{error}</div> : null}
...
function AdminPage(){
 ...
 async function loadAdmin(){ if(!session){setMessage("관리자 로그인이 필요합니다.");return;} const {data,error}=await supabase.from("instructors").select("*").order("created_at",{ascending:false}); if(error)setMessage("조회 실패: "+error.message); else setItems(data||[]); }
